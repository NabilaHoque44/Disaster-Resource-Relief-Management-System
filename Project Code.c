...
int totalResource = 0; int totalVictim = 0;
...
/* Relief Distribution */
void saveDistributions();//extra add
void loadDistributions();//extra add
...
void searchResource()
{
 ...
 printf("Name : %s\n", resource[i].name);
 printf("Category : %s\n", resource[i].category);
 printf("Quantity : %d\n", resource[i].quantity); printf("Unit : %s\n", resource[i].unit);
 return;
 }
 }
 printf("\nResource Not Found.\n");
...
void updateVictim()
{
 ...
 printf("New Area: ");
 scanf(" %[^\n]", victim[i].area);
 printf("New Priority(High/Medium/Low): ");
 scanf(" %[^\n]", victim[i].priority);
...
void searchVolunteer()
{
 ...
 printf("\nVolunteer Found!\n");
 printf("ID : %d\n", volunteer[i].id);
 printf("Name : %s\n", volunteer[i].name); printf("Phone : %s\n", volunteer[i].phone);
 printf("Area : %s\n", volunteer[i].area);
 return;
 }
 }
...
/* ==============================
 Save Distribution (extra add)
 ============================== */
void saveDistributions()
{
    FILE *fp;

    fp = fopen("distributions.dat", "wb");
...
void loadDistributions()
{
    ...
    totalDistribution = fread(distribution,
    sizeof(struct Distribution),
    MAX_DISTRIBUTION,
    fp);
